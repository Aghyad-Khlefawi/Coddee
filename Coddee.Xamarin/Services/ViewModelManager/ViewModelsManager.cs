...
namespace Coddee.Xamarin.Services.ViewModelManager
{
    public class ViewModelsManager : IViewModelsManager
    {
        ...
        {
            if (!string.IsNullOrEmpty(viewModel.ViewModelGroup) &&
                _viewModelGroups.ContainsKey(viewModel.ViewModelGroup))
                _viewModelGroups[viewModel.ViewModelGroup].Remove(viewModel);

            var list = _viewModelGroups.ContainsKey(group)
                ? _viewModelGroups[group]
                : (_viewModelGroups[group] = new List<IViewModel>());
            list.Add(viewModel);
        }
        ...
        public TResult CreateViewModel<TResult>(IViewModel parentVM, ViewModelOptions viewModelOptions)
            where TResult : IViewModel
        {
